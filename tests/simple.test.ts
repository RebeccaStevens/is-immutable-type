...
describe("primitives", () => {
  it.each(["null", "undefined", "string", "number", "boolean", "symbol", "bigint"])("treat %s as immutable", (test) => {
    runTestImmutability(`type Test = ${test};`, Immutability.Immutable);
  });
});

describe("records", () => {
  it.each(["type Test = { readonly foo: string; };", "type Test = Readonly<{ foo: string; }>;"])(
  ...
  it.each(["type Test = { foo: string; };"])("handles mutable records", (code) => {
...
describe("arrays", () => {
  ...
  it.each(["type Test = readonly string[];", "type Test = ReadonlyArray<string>;"])(
  ...
  it.each(["type Test = readonly { foo: string }[];", "type Test = ReadonlyArray<{ foo: string }>;"])(
  ...
  it.each(["type Test = string[];", "type Test = Array<string>;"])("handles mutable arrays", (code) => {
    runTestImmutability(code, Immutability.Mutable);
  });
});

describe("tuples", () => {
  it.each(["type Test = readonly [string, number, boolean];", "type Test = Readonly<[string, number, boolean]>;"])(
  ...
  it.each(["type Test = [string, number, boolean];"])("handles mutable tuples", (code) => {
    runTestImmutability(code, Immutability.Mutable);
  });
});

describe("sets and maps", () => {
  it.each(["type Test = Readonly<ReadonlySet<string>>;", "type Test = Readonly<ReadonlyMap<string, string>>;"])(
  ...
  it.each(["type Test = ReadonlySet<string>;", "type Test = ReadonlyMap<string, string>;"])(
  ...
  it.each(["type Test = ReadonlySet<{ foo: string }>;", "type Test = ReadonlyMap<{ foo: string }, { bar: string }>;"])(
  ...
  it.each(["type Test = Set<string>;", "type Test = Map<string, string>;"])("handles mutable sets and maps", (code) => {
    runTestImmutability(code, Immutability.Mutable);
  });
});
...
describe("methods", () => {
  ...
  it.each(["type Test = { foo: () => string; };", "type Test = { foo(): string; };"])(
